...
class sendMailBaoKimNotification extends Job
{
    /**
    ...
    public function __construct($request,$money=null)
    {
    ...
    {
        $order_id = explode('-add_money-', $this->request['order_id']);
        if (isset($order_id[1])) {
            $user = User::find($order_id[1]);
            if($user){
                if($user->use_email!=$this->request['customer_email']){
                    $this->address_cc = $this->request['customer_email'];
                }
            }

            $transaction_status = $this->request['transaction_status'];
            if (key_exists($transaction_status, $this->arr_transaction_status)) {
                $status_body = $this->arr_transaction_status[$transaction_status];
                ...
                $status_body = 'Giao dịch không thành công';
...
//            $mail->addAttachment('/tmp/image.jpg', 'new.jpg');    // Optional name
                ...
                $mail->Body = $this->renderBody($user, $this->request['order_id'],$status_body);
    ...
    protected function renderBody($user,$order_id,$status)
    {
        ...
        <div style="display:table;font-size:13px;padding-left:8px;padding-bottom:2px">
            <table style="border-spacing:2px;text-indent:0;border-collapse:collapse">
                ...
                <tr style="display:table-row;vertical-align:inherit">
                    ...
                    <td style="border:1px solid #aaaaaa;padding:8px 75px" colspan="3">'.number_format($money,0,",",".").' XU </td>
                </tr>
                <tr style="display:table-row;vertical-align:inherit">
                    <td style="border:1px solid #aaaaaa;padding:8px 15px"> <b>Số XU nạp vào</b> </td>
                    <td style="border:1px solid #aaaaaa;padding:8px 75px;text-transform:uppercase" colspan="3">'.number_format($this->request['order_amount'],0,",",".").' XU </td>
                </tr>
                <tr style="display:table-row;vertical-align:inherit">
