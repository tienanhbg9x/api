...
class SendMailPaymentToUser extends Job
{
    ...
    {
        $this->email = arrGetVal('email',$data,null);
        $this->ush_id = arrGetVal('ush_id',$data,null);
    }
    ...
    public function handle()
    {
        ...
        if ($ush) {
            ...
            if($user){
                if($this->email!=null&&$user->use_email!=$this->email){
                    $this->address_cc = $this->email;
                }
            }

            $transaction_status = $ush->ush_status;
            ...
            try {
                ...
                if($this->address_cc!=null&&$this->address_cc!=$user->use_email_payment){
                    $mail->addCC(trim($user->address_cc));
                }
...
//            $mail->addAttachment('/tmp/image.jpg', 'new.jpg');    // Optional name
                ...
                $mail->Body = $this->renderBody($user,$ush);
                $mail->AltBody = 'Email send from website: https://sosanhnha.com';
                $mail->send();
            } catch (\Exception $e) {
                sendNotifySlack(['ush_id'=>$this->ush_id,'type'=>'momo_send_mail','status'=>'fail','message'=>$e->getMessage(),'mail_exception'=>$mail->ErrorInfo],'https://hooks.slack.com/services/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');
                @file_put_contents(storage_path('logs/mail_log.log'),date('d-m-Y H:i:s').':'.'PHPMAILER(notification baokim):' . $mail->ErrorInfo. "\n",FILE_APPEND);
            }
        }
    }

    protected function renderBody($user,$ush)
    {
    ...
    <div style="display:table;font-size:12px;line-height:18px;font-family:arial">
        ...
        <div style="display:table;color:#333333;padding-left:8px;font-family:arial">
            <p style="font-size:13px">
                ...
                Quý khách thực hiện giao dịch nạp XU vào tài khoản với mã giao dịch '.$ush->ush_order_id.' trên website <a href="https://sosanhnha.com" target="_blank" data-saferedirecturl="https://www.google.com/url?q=https://sosanhnha.com&amp;source=gmail&amp;ust=1544782641526000&amp;usg=AFQjCNFqbPaPMh3gQApKbbYcXTDrm1ZCUQ">https://sosanhnha.com</a> .<br>
        ...
        <div style="display:table;font-size:13px;padding-left:8px;padding-bottom:2px">
            <table style="border-spacing:2px;text-indent:0;border-collapse:collapse">
                ...
                <tr style="display:table-row;vertical-align:inherit">
                    ...
                    <td style="border:1px solid #aaaaaa;padding:8px 75px" colspan="3">  '.$ush->ush_order_id.' </td>
                </tr>
