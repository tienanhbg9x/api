...
class sendMailChangeMoneyUser extends Job
{
    /**
    ...
    public function handle()
    {
        try {
            ...
            $this->money = Money::where('mon_user_id',$this->request['user_id'])->first();
            $this->ush = UserSpendHistory::find($this->ush);
            $this->user = $user;
            $subject = 'Thông báo số dư XU trong tài khoản thay đổi  - Website bất động sản https://www.sosanhnha.com';
            ...
            $user_email = strpos($user->use_email,"@facebook")===false?$user->use_email:$user->use_email_payment;
            $mail->addAddress(trim($user_email), $user->use_name);     // Add a recipient
...
//            $mail->addBCC('minhngoc2512@yahoo.com');
            ...
            @file_put_contents(storage_path('logs/mail_log.log'), date('d-m-Y H:i:s') . ':' . 'PHPMAILER(notification money update): ' . $mail->ErrorInfo . "\n", FILE_APPEND);
        }
    }
        ...
        $body = '
                <div style="margin: 0">
                        <div style="display:table;font-size:12px;line-height:18px;font-family:arial">
                            ...
                            <div style="display:table;padding-left:8px;font-family:arial">
                                <p style="font-size:13px">
                                    Kính gửi: <span>  ' . $user->use_loginname . '</span> <br>
                                    ID tài khoản của bạn: <strong>  ' . $user->use_id . '</strong> <br>
                                    Hệ thống <a href="https://www.sosanhnha.com/" target="_blank" >sosanhnha.com</a> xin thông báo số dư tài khoản được cập nhật.<br>
                                    Lý do cập nhật: <strong style="color: red">'.$this->request['message'].' </strong><br>
                                    Mã người cập nhật: '.$this->request['user_admin_id'].'
                                </p>
                                <p>
                            ...
                            <div style="display:table;font-size:13px;padding-left:8px;padding-bottom:2px">
                                <table style="border-spacing:2px;text-indent:0;border-collapse:collapse">
                                    ...
                                    <tr style="display:table-row;vertical-align:inherit">
                                        ...
                                        <td style="border:1px solid #aaaaaa;padding:8px 75px" colspan="3"> ' . number_format((int)$this->ush->ush_count, 0, '', ',') . '  XU</td>
                                    </tr>
                                                 <tr style="display:table-row;vertical-align:inherit">
                                        <td style="border:1px solid #aaaaaa;padding:8px 15px"> <b>Số dư tài khoản hiện tại:</b> </td>
                                        <td style="border:1px solid #aaaaaa;padding:8px 75px" colspan="3"> ' .  number_format($this->money->mon_count, 0, '', ',')  . ' XU  </td>
                                    </tr>
                                              <tr style="display:table-row;vertical-align:inherit">
                            ...
                            <div style="display:table;color:#333333;text-align:left;width:650px;padding-top:5px;padding-left:20px;font-size:13px;margin-bottom:7px">
                               <i>Mọi thắc mắc bạn vui lòng trả lời mail ' . config('phpmailer.user_name') . ' - ngocnm@vatgia.com hoặc liên hệ với quản trị trang soanhnha.com: <a href="tel: +84915356965">0915356965</a> .
                                </i>
                                <p>
