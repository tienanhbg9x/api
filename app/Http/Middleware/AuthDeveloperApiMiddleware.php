...
class AuthDeveloperApiMiddleware
{
    /**
    ...
    public function handle($request, Closure $next)
    {
        if($request->input("access_token")==null){
            return response()->json(["status_code"=>401,"message"=>"Error!(middleware)"],401);
        }
        try{
            $user_info =   JWT::decode($request->input("access_token"),config("app.secret_access_token"),['HS256']);
            if($user_info->data->rol!="admin"&&$user_info->data->rol!="developer"){
