...
class BaoKimPayment
{
    ...
    public function createRequestUrl($data)
    {
        ...
        $params = array(
            'merchant_id'		=>	strval(BAOKIM_MERCHANT_ID),
            'order_id'			=>	strval($order_id),
            'business'			=>	strval(BAOKIM_EMAIL_BUSINESS),
        ...
        @file_put_contents(storage_path('logs/bao_kim_send.log'),date('d-m-Y H:i:s').':'.json_encode($params,JSON_UNESCAPED_UNICODE,JSON_PRETTY_PRINT) . "\n",FILE_APPEND);
        ksort($params);

        $params['checksum']=hash_hmac('SHA1',implode('',$params),BAOKIM_SECURE_PASS);
    ...
    public function verifyResponseUrl($url_params = array())
    {
        ...
        if(strcasecmp($checksum,hash_hmac('SHA1',implode('',$url_params),BAOKIM_SECURE_PASS))===0)
            return TRUE;
        else
            return FALSE;
    }
